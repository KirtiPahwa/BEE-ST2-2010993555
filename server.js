...
app.post("/signup-form", (req, res) => {
  ...
  var loggedIn = 1;
  const getEach = "select id from student where username=?";
  const PostEach =
    "INSERT INTO student (username,password,name,contact,gender,log) values(?,?,?,?,?,?);";
  db.query(getEach, [username], (err, result) => {
    if (result.length == 0) {
      db.query(
        PostEach,
        [username, password, name, contact, gender, 0],
        (err, result) => {
          console.log(result);
          res.redirect("/login");
        }
...
app.post("/login-form", (req, res) => {
  ...
  const getEach = "select id from student where username=? and password=?;";
  db.query(getEach, [username, password], (err, result) => {
    if (result.length == 0) {
      res.redirect("/login");
    } else {
      const post = "update student set log=? where username=?;";
...
app.post("/logout-form", (req, res) => {
  const checkLogin = "update student set log=? where log=?";
...
app.get("/admin-get", (req, res) => {
  const checkLogin = "select * from student;";
...
app.get("/", (req, res) => {
  const checkLogin = "select * from student where log=?;";
...
app.post("/change-pass-form", (req, res) => {
  ...
  db.query(checkLogin, [1], (err, result) => {
    if (oldpass === result[0].password) {
      if (newpass === conpass) {
        const updatePass = "update student set password=? where log=?;";
